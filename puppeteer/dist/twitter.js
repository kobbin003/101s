...
const getStatusContent = async (page, username) => {
    console.log("get status content");
    const tweetXPath = '//article[@data-testid="tweet"]';
    await page.waitForXPath(tweetXPath);
    const tweetProfilePicDivXPath = '//article[@data-testid="tweet"]/div/div/div[2]/div[1]';
    const tweetNamesDivXPath = '//article[@data-testid="tweet"]/div/div/div[2]/div[2]/div[1]';
    const tweetTextDivXPath = '//article[@data-testid="tweet"]/div/div/div[3]/div[1]';
    const tweetImgDivXpath = '//article[@data-testid="tweet"]/div/div/div[3]/div[2]/div/div[1]';
    const tweetLinkedDivXpath = '//article[@data-testid="tweet"]/div/div/div[3]/div[2]/div/div[2]';
    /* ----------------------------------- tweet div ---------------------------------- */
    ...
    if (tweetDivs.length > 0) {
        ...
        if (profileImageDivs.length > 0) {
            const profileImage = await profileImageDivs[0].$$(`div[data-testid="UserAvatar-Container-${username}"] img`);
        ...
        if (namesDivs.length > 0) {
            const fullNames = await namesDivs[0].$$('div[data-testid="User-Name"] div span > span');
            if (fullNames.length > 0) {
                const fullNameStatus = await page.evaluate((el) => el.textContent, fullNames[0]);
        ...
        if (tweetImageDivs.length > 0) {
            ...
            const tweetPhotos = await tweetImageDivs[0].$$("div[data-testid='tweetPhoto'] img");
        ...
        if (linkedTweetDiv.length > 0) {
            ...
            const profileImageLinkTweet = await linkedTweetDiv[0].$$(`div[data-testid="Tweet-User-Avatar"] img`);
            ...
            const usernames = await linkedTweetDiv[0].$$('div[data-testid="User-Name"] > div:nth-of-type(2) span');
            // console.log("allNames", allNames);
            if (usernames.length > 0) {
                const usernameLinkedTweet = await page.evaluate((el) => el.textContent, usernames[0]);
                console.log(`usernameLinkedTweet`, usernameLinkedTweet);
            }
            // /* ------------------------------- linked tweet full name ------------------------------- */
            const fullNames = await linkedTweetDiv[0].$$('div[data-testid="User-Name"] div:nth-of-type(1) span > span');
            if (fullNames.length > 0) {
                const fullNameStatus = await page.evaluate((el) => el.textContent, fullNames[0]);
                console.log("linkedfullNameStatus", fullNameStatus);
            }
            /* --------------------------- linked tweet photos --------------------------- */
            const linkedTweetPhotos = await linkedTweetDiv[0].$$("div[data-testid='tweetPhoto'] img");
...
const runX = async () => {
    try {
        ...
        await navigateTo(page, "https://twitter.com/kkob03/status/1713527996722635185");
